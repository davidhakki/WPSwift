...
extension Post {
    static var mockDataWithNoEmbedded: Data {
        get throws {
            guard let url = Bundle.module.url(forResource: "PostNoEmbedded", withExtension: "json") else {
                throw MockError.fileNotFound("PostNoEmbedded.json")
            }
            let data = try Data(contentsOf: url)
            return data
        }
    }
    static var mockFromDataWithNoEmbedded: Post {
        get throws {
            let data = try mockDataWithNoEmbedded
            let decoder = JSONDecoder.initialize()
            let post = try decoder.decode(Post.self, from: data)
            return post
        }
    }
    static var mockWithNoEmbedded: Post {
        .init(
            id: 1234,
            date: DateFormatter.default.date(from: "2023-10-06T14:08:00"),
            modified: DateFormatter.default.date(from: "2023-10-06T14:08:00"),
            ...
            tags: [3, 4],
            link: "https://example.com/1234",
            embeddedContent: .mockWithNoEmbedded
        )
    }
}
